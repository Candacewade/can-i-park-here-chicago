...
def list_watches_by_email(email: str = Query(min_length=3)) -> WatchesByEmailResponse:
    """Every ACTIVE watch registered to this email address, each with its own
    manage_token so the frontend can extend/stop it directly.

    NOT verified: knowing the address is enough, by explicit user request
    (an emailed-link verification step was tried first and dropped because
    the confirmation email wasn't arriving reliably). This means anyone who
    knows or guesses an email address used with this app can view and cancel
    its watches -- see docs/monitoring.md for the trade-off this accepted."""
